...
async def get_pending_kyc_submissions(
    ...
    # Query only pending submissions
...
async def verify_kyc_submission(
    ...
    if kyc.get('status') != 'pending':
        raise HTTPException(
            ...
            detail=f"KYC is already {kyc.get('status')}"
        )
    ...
    update_data = {
        ...
        "reviewed_by": current_admin.get('email'),
    ...
    if result.modified_count == 0:
        raise HTTPException(
            ...
            detail="Failed to update KYC submission"
        )
    
    # Update user's KYC status
    await db.users.update_one(
        {"id": kyc['user_id']},
        {"$set": {"kyc_status": new_status, "kyc_verified_at": reviewed_at}}
    )
    ...
    return {
        "message": f"KYC {new_status} successfully",
        "kyc_id": kyc_id,
        "status": new_status,
        "reviewed_at": reviewed_at
    }
