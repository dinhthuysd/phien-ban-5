...
@router.put("/users/{user_id}/premium", response_model=MessageResponse)
async def toggle_user_premium(
    ...
    await log_audit(
        db, current_admin['id'], "user_premium_updated",
        {"user_id": user_id, "is_premium": is_premium},
        request.client.host if request else None,
        request.headers.get("user-agent") if request else None
    )
    
    return MessageResponse(message=f"User premium {'enabled' if is_premium else 'disabled'} successfully")
